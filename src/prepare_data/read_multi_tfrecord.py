...
def read_multi_rd(data_record_dir,fg_name,bg_name,batch_size,ratio=0.25):
    fg_batch = int(np.floor(batch_size * ratio))
    bg_batch = batch_size - fg_batch
    fg_rd = Read_Tfrecord(cfgs.DATASET_NAME,data_record_dir,fg_batch,fg_name)
    fg_name_batch, fg_img_batch, fg_label_batch = fg_rd.next_batch()
    bg_rd = Read_Tfrecord(cfgs.DATASET_NAME,data_record_dir,bg_batch,bg_name)
...
if __name__ == '__main__':
    img_dict = dict()
    sess = tf.Session()
    img_batch, gtboxes_and_label_batch = read_multi_rd('../../data','fg','bg',4)
    '''
