...
def detect_face_12net(cls_prob,roi,out_side,scale,width,height,threshold):
    #print("cls and roi",cls_prob.shape,roi.shape)
    ...
    #print("pnet bounding and score ",np.shape(boundingbox),score.shape)
...
def detect_face_12net_(cls_prob,roi,out_side,scale,width,height,threshold):
    in_side = 2*out_side+11
    stride = 0
    if out_side != 1:
        stride = float(in_side-12)/(out_side-1)
...
def filter_face_48net(cls_prob,roi,pts,rectangles,width,height,threshold):
    ...
    for i in range(len(rectangles)):
        ...
        x2 = int(min(width ,rectangles[i][2]))
        y2 = int(min(height,rectangles[i][3]))
        if x2>x1 and y2>y1 and config.x_y:
            pick.append([x1,y1,x2,y2,rectangles[i][4],rectangles[i][5],rectangles[i][6],rectangles[i][7],rectangles[i][8],rectangles[i][9],rectangles[i][10],rectangles[i][11],rectangles[i][12],rectangles[i][13],rectangles[i][14]])
        if x2>x1 and y2>y1 and not config.x_y:
            pick.append([x1,y1,x2,y2,rectangles[i][4],rectangles[i][5],rectangles[i][7],rectangles[i][9],rectangles[i][11],rectangles[i][13],rectangles[i][6],rectangles[i][8],rectangles[i][10],rectangles[i][12],rectangles[i][14]])
...
Output:
    ...
    scales  : Multi-scale
'''
def calculateScales(img,min_size):
    caffe_img = img.copy()
    #pr_scale = 1.0
    ...
    #multi-scale
    netsize = 12
    pr_scale = float(netsize)/min_size
    scales = []
    factor = 0.709
    factor_count = 0
    h = int(h*pr_scale)
